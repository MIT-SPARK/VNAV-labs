...
class TesseROSWrapper:
    ...
    def clock_cb(self, event):
        ...
        if self.publish_clock or self.publish_collisions:
            try:
                ...
                if self.publish_clock:
                    curr_ros_time = rospy.Time.from_sec(metadata['time'])
                    self.clock_pub.publish(curr_ros_time)
    ...
    def reposition_cb(self, msg):
        ...
        quat = tf.transformations.quaternion_multiply([-qx, -qz, -qy, qw],
    ...
    def meta_cb(self, data):
        ...
        timestamp = rospy.Time.from_sec(metadata_processed['time'])
    ...
    def image_cb(self, event):
        ...
        try:
            ...
            timestamp = rospy.Time.from_sec(metadata['time'])
    ...
    def object_cb(self,event):
        ...
        for id, obj in obj_dict.items():
            ...
            self.tf_broadcaster.sendTransform(obj['position'],
                                              obj['quaternion'],
                                              rospy.Time.from_sec(obj['time']),
                                              frame_id,
                                              self.world_frame_id)
    ...
    def setup_all_lidars(self):
        ...
        """
        lidar_params = rospy.get_param("~lidar_params")
